...
export function HarmonogramWeek(props) {

    const cls = useStyles()

    // height multiplier. 
    // effective content height will be that * 24
    let _hm = props.sm ? 20 : 22
    const hm = _hm
    ...
    function setOverlaps(o) {
        ...
        for (let k in o) {
            ...
            for (let i = 0; i < d.length; i++) {
                ...
                if (f) continue
                el.style.width = cw - el.style.marginLeft - marginLeftOffset
            }
    ...
    function dayBlockStyle(hr, hhr, _off) {
        return {
            ...
            width: Math.round(cw - 2 * (cw / 30)),
            marginLeft: Math.round(cw / 40),
            color: "white",
            textAlign: "left",
            cursor: "pointer",
            marginTop: Math.round((_off || off) + (hr * hm) - 6),
            overflow: "hidden",
        ...
        for (let i = 0; i < s.length; i++) {
            ...
            for (let j = 0; j < s[i].Schedule.length; j++) {
                ...
                for (let j = 0; j < sch.Occ.SecondaryOccs.length; j++) {
                    ...
                    a[day2].push({ mday: true, style: secondaryOccStyle, noBoxShadow: noBoxShadow })
                }
        ...
        if (+now >= +props.week.start && +now <= +props.week.end) {
            a[nowday].push({
                style: {
                    ...
                    paddingLeft: 4,
                    borderRadius: 3,
                    marginTop: off + ((now.getHours() + (now.getMinutes() / 60)) * hm),
                },
            })
        }
            ...
            <Grid container direction="column"  onClick={e => {
                ...
                {day != -1 && (
                    <Typography align="center"><span style={{ color: "gray" }}>
                        {dd.getDate()} / </span>{day} 
                        {/* {!props.readonly && props.editorData && <IconButton size="small" 
                            onClick={() => {
                                let [occs, indexes] = addOcc2(props.editorData.occs, lastAdded, dd, props.repeating)
                                let ed = { ...props.editorData }
                                ed.occs = occs
                                props.setEditorData(ed)
                                setEditIndexes(indexes)
                ...
                {dis && dis.map((di, i) => {
                    ...
                    if (di.style.width <= (cw / 2)) {
                        fs = 9
                    }
                    if (di.style.width <= (cw / 3)) {
                        fs = 7
                    }
                    ...
                    return (<React.Fragment key={i}>
                        <HtmlTooltip title={(di.sch && di.t && (<React.Fragment>
                            ...
                            <Box className={cls.cardItem} boxShadow={di.noBoxShadow ? null : 3} style={di.style} onClick={(e) => {
                                ...
                                {/* {!props.usrRsv && di.sch && (
                                        <div style={{
                                            ...
                                            width: 10,
                                            height: 10,
                                            backgroundColor: di.sch.IsOrphaned ? "gray" : di.sch.IsAvailable ? MulwiColors.greenDark : MulwiColors.redError
                                        }}>
                                        </div>
                                    )} */}
    ...
    function DD() {
        return (<Divider style={{
            height: 2,
            borderColor: theme.palette.grey[300],
            marginTop: -8 + soff, position: "absolute", width: cw}} />)
    }

    // mday = whether occ has subOccs ( true )
    // mdayEdit = props parsed from weekBigRes into child
    ...
    return (
        <React.Fragment>
            {mdayEditModal()}
            <OpPicker 
                open={opPickerOpen} setOpen={setOpPickerOpen}
                setOp={setOp}
                lang={props.lang}
            />
            <Grid container
